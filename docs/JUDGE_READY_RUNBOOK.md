...
7. Keep `KEEPERHUB_ALLOWED_NETWORKS=sepolia` for judging unless Base has been dry-run.
8. Keep `KEEPERHUB_MAINNET_CONFIRMED=false` unless intentionally enabling Base/mainnet writes.
...
4. Confirm the dashboard does not show the operator/default organism.
5. Create organism.
6. Confirm execution wallet or sponsored-start state is visible.
7. Open Dashboard.
8. Click Run now.
9. Confirm activity, storage records, iNFTs, KeeperHub rows, and treasury are scoped to the new organism.
10. Switch wallet and confirm old organism state clears.
