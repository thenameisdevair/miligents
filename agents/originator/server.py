...
def trigger_run(payload: RunRequest | None = None):
    ...
    def _run():
        ...
        try:
            ...
            write_agent_status("originator", "error", result=str(e), organism_id=organism_id)
            write_activity("originator", "error", f"run() raised: {str(e)[:120]}", cycle_id=cycle_id, organism_id=organism_id)
        finally:
            _is_running = False

    thread = threading.Thread(target=_run, daemon=True)
    thread.start()
    return {"status": "started", "agent": "originator", "organism_id": (payload.organism_id if payload else DEFAULT_ORGANISM_ID)}
