...
def store_research_tool(data: str, filename: str) -> str:
    ...
    try:
        ...
        write_storage_record("originator", filename, root_hash)
        write_agent_status("originator", "running", current_task=f"Stored research: {filename}")
...
def receive_axl_tool(dummy: str = "") -> str:
    ...
    try:
        ...
        formatted = []
        for m in messages:
            formatted.append(_clip(json.dumps(m, indent=2), 600))
        return _clip("\n---\n".join(formatted))
    except Exception as e:
...
@tool("get_our_pubkey")
@emit(agent_id=AGENT, summary_fn=lambda a, k: "reading AXL public key")
