"""
agents/execution/server.py

FastAPI wrapper for the Execution Agent.
Exposes POST /run and GET /status so the scheduler
can trigger and poll this agent over the Docker network.

Runs on port 8003 (internal to Docker network only).
"""
...
@app.get("/status")
def status():
    return {"agent": "execution", "running": _is_running}
...
def trigger_run():
    ...
    with _run_lock:
        if _is_running:
            raise HTTPException(status_code=409, detail="Execution agent already running")
        _is_running = True
